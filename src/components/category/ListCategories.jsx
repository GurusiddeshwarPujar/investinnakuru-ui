...
export default function CategoryTable({ categories, onEditClick, onCategoryDeleted, onError, handleAuthError }) {
  ...
  const handleDelete = async (catId) => {
    if (window.confirm("Are you sure you want to delete this key sector?")) {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(`Failed to delete key sector: Server responded with status ${res.status}. Please check your backend API.`);
        }
  ...
  return (
    <>
      <h2 className="text-2xl font-bold mb-4">List of Key Sectors</h2>
      <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-3">
        ...
        <input
          type="text"
          placeholder="Search key sectors..."
          value={search}
                ...
                <tr key={category.CatId} className="border-b">
                  <td className="py-3 px-4">
                    <div className="w-16 h-16 flex-shrink-0 flex items-center justify-center">
                      {category.Image && (
                        <img
                          src={`${backendUrl}/images/keysector/${category.Image}`}
      ...
      <div className="flex justify-between items-center mt-4">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          ...
          {filteredCategories.length} Key sectors
        </p>
